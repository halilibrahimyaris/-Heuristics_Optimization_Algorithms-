...
public class Solution implements Cloneable{
    ...
    public int checkFeasibility(){
        ...
        int cap = 0;
        for (Item item : rndSolution) {
            //seçilmiş olan elemanların ağırlıklarını topluyoruz.
            cap += (item.getIsSelected() * item.getWeight());
        }
        //feasibility kontrolü 
        if (Utils.knapsackSize <= cap){
            return PENALTY_VALUE;
            //else state
        } else {
            return (int) (obj - PENALTY_VALUE);
        }
    }
